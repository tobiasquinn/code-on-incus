...
- **Default `protected_paths` no longer include the claude/codex settings files (fork)** — this fork runs opencode/pi only, so the anti-planting placeholders for `.claude/settings.json`, `.claude/settings.local.json` and `.codex/config.toml` — which auto-created `.claude/` and `.codex/` directories in every workspace — are removed from the defaults. The protection mechanism itself is unchanged (the paths stay in `fileTypeProtectedPaths`/`fileTypeParentAutoCreate`); if you run claude or codex on the host against these workspaces, restore the protection in trusted-scope config: `[security] additional_protected_paths = [".claude/settings.json", ".claude/settings.local.json", ".codex/config.toml"]`.
...
- **`~/.claude` and `~/.codex` are no longer baked into every image (#454 follow-up)** — `create_code_user()` in build.sh created both directories unconditionally, so an image built with a restricted agent set (e.g. `COI_AGENTS="opencode pi"` / `agents = ["opencode", "pi"]`) still shipped config dirs for agents it never installed. The dirs are now created by their installers (`install_claude_cli` / `install_codex`, owned by the code user so the CLI can write state), so they exist in the image exactly when the agent does; runtime session setup still creates them on demand when needed.
